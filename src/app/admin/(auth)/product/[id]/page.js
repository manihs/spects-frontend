                ...
                if (productResponse.success && productResponse.data) {
                    ...
                    if (product.attributes && product.attributes.length > 0) {
                        const formattedAttributes = product.attributes.map(attr => {
                            ...
                            // Handle multiple_select attributes (stored as JSON string)
                            if (attr.attribute.type === 'multiple_select' && typeof attr.value === 'string') {
                                try {
                    ...
                    if (product.hasVariants && product.variants && product.variants.length > 0) {
                        ...
                        // Extract variant attribute IDs - find all unique attribute IDs across all variants
                        const varAttrIds = new Set();
                        const varAttrMap = new Map();

                        product.variants.forEach(variant => {
                            if (variant.attributes && variant.attributes.length > 0) {
                                variant.attributes.forEach(attr => {
                                    varAttrIds.add(attr.attributeId);
                                    varAttrMap.set(attr.attributeId, {
                                        attributeId: attr.attributeId,
                                        name: attr.attribute.name,
                                        type: attr.attribute.type,
                                        options: attr.attribute.options || []
                        ...
                        setVariantAttributes(Array.from(varAttrMap.values()));
                    ...
                    <nav className="flex mb-3" aria-label="Breadcrumb">
                        <ol className="flex items-center space-x-2 text-sm text-gray-500">
                            ...
                            <li className="flex items-center">
                                ...
                                <span className="ml-2 font-medium text-blue-600">Edit Product</span>
                            </li>
                        </ol>
                    </nav>
